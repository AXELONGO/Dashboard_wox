...
interface HeaderProps {
  ...
  onTabChange: (tab: 'ventas' | 'cotizaciones' | 'clientes' | 'masivos') => void;
  onLogout: () => void;
}
  ...
  return (
    <header className="flex-none flex items-center justify-between whitespace-nowrap border-b border-glass-border bg-black/20 backdrop-blur-xl px-4 md:px-6 py-4 z-20">
      <div className="flex items-center gap-4 text-white">
        {/* Mobile Menu Button - Always show on mobile to access History/Support sidebar */}
        <button
      ...
      <div className="flex flex-1 justify-end gap-2 md:gap-4 items-center overflow-x-auto no-scrollbar">
        <nav className="flex items-center gap-2 lg:gap-8 overflow-x-auto no-scrollbar mask-gradient-x p-1">
          <button
          ...
          <button
            onClick={() => onTabChange('masivos')}
            className={`text-sm font-medium leading-normal transition-all hover:text-glow px-2 py-1 relative ${activeTab === 'masivos' ? 'text-white' : 'text-gray-400 hover:text-white'} `}
          >
            Masivos
            {activeTab === 'masivos' && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-white shadow-glow rounded-full"></span>}
          </button>
