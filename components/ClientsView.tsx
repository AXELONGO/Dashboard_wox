...
import { createClientInNotion } from '../services/notionService';
...
const ClientsView: React.FC<ClientsViewProps> = ({ clients, toggleSelectClient, onClassChange }) => {
    ...
    const selectedCount = clients.filter(c => c.isSelected).length;
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-transparent relative z-0 h-full overflow-hidden">
            ...
            <div className="p-4 md:p-6 shrink-0 z-10">
                <div className="glass-panel rounded-2xl p-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-4 shadow-glass">
                    ...
                    <button
                        ...
                        className="p-2 md:px-4 md:py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20 active:scale-95 group border border-white/5 order-3 md:order-3"
                        title="Nuevo Cliente"
                    >
                        <span className="material-symbols-outlined text-xl group-hover:rotate-90 transition-transform">add</span>
                        <span className="hidden md:inline font-bold">Nuevo Cliente</span>
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 md:px-6 pb-20 scroll-smooth">
                <div className="flex flex-col gap-3">
                    {/* Header of List */}
                    <div className="flex items-center px-6 py-2 text-[10px] font-bold text-white uppercase tracking-widest hidden sm:flex">
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex flex-wrap items-center gap-3 mb-1.5">
                                    <h4 className={`font-semibold text-sm md:text-base truncate max-w-full transition-all ${client.isSelected ? 'text-white text-glow scale-[1.01]' : 'text-white'}`}>
                                        {client.name}
                                    </h4>
                                    <a className="text-white hover:text-gray-300 transition-colors flex items-center p-1 rounded hover:bg-white/10" href={client.website || '#'} target="_blank" rel="noreferrer">
                            ...
                            <div className="w-48 hidden lg:flex flex-col justify-center gap-1">
                                <div className="flex items-center gap-2 text-white text-sm">
                                    ...
                                    <span className="font-mono text-xs hover:text-gray-300 transition-colors cursor-text selection:bg-white/20">{client.phone}</span>
                                </div>
                                <div className="text-xs text-white truncate hover:text-gray-300 cursor-pointer transition-colors pl-6 hover:underline">{client.website || 'No website'}</div>
                            </div>
            ...
            <AddClientModal
                ...
                onSave={async (clientData) => {
                    const newClient = await createClientInNotion(clientData);
                    if (newClient) {
